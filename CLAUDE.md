...
For release and publishing work, follow `RELEASE.md` plus the `Release And Publishing` and `Release Truth Rules` sections in `AGENTS.md`. Verify `@unship` npm org/scope access before publishing, and keep package contents constrained by `test/package-smoke.test.js`.
