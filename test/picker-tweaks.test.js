...
test("tweak-only groups render without chevrons or counter", async () => {
  const browser = await chromium.launch();
  try {
    const page = await browser.newPage({ viewport: { width: 800, height: 600 } });
    await page.setContent(`
<nav data-unship-pick="Navigation" style="--nav-pad: 16px;">
  <div data-unship-option="Current" style="--accent: #0071e3;" data-unship-tweaks='[{"type":"slider","label":"Padding","var":"--nav-pad","min":8,"max":32,"step":2,"unit":"px"},{"type":"swatch","label":"Accent","var":"--accent","options":[{"label":"Sky","value":"#0071e3"},{"label":"Graphite","value":"#1d1d1f"}]}]'>links</div>
</nav>
<script>${picker}</script>`);
    await page.waitForFunction(() => document.querySelector("[data-unship-toolbar]")?.shadowRoot?.querySelector(".dock"));
    ...
    const state = await page.locator("css=[data-unship-toolbar]").evaluate((host) => {
      ...
      return {
        ...
        tune: Boolean(root.querySelector(".tune")),
        label: root.querySelector(".label-main").textContent
      };
    });
